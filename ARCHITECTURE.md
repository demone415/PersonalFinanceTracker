...
**Story 6.3: Глобальное отслеживание фоновых задач на фронте _(на потом)_**
- [ ] T6.3.1 Глобальный стейт активных задач экспорта/импорта (`BackgroundTask`), не привязанный к странице. Сейчас `jobId` живёт в локальном стейте `ExportButton` (`useAccrualExport`), поэтому при уходе со страницы фронт «забывает» задачу: опрос статуса прекращается, тост о готовности и автоскачивание не срабатывают (задача на бэке при этом доезжает).
  - Хранить список активных `jobId` вне компонента (Zustand + `localStorage` или URL), регидрировать при загрузке приложения; провайдер опроса (`useJobStatus`) поднять на уровень `AppProviders`, чтобы статусы обновлялись на **любой** странице.
  - Уведомлять пользователя глобально (тост о `Done`/`Failed`, авто-скачивание готового файла; опционально — индикатор «активные задачи» в шапке) независимо от текущего маршрута.
  - Переживать перезагрузку вкладки (восстановление по сохранённым `jobId` через `GET /jobs/{id}`) и охватывать как экспорт (6.2), так и импорт (6.1.4).
