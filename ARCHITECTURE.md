...
> валюты позже не пересчитывает исторические записи. Курс **обязателен**, когда
> валюта операции отличается от основной: это требует и форма, и `AccrualService`
> на сервере (операция в иностранной валюте без курса → 400), иначе агрегаты молча
> посчитали бы её 1:1.
...
├── Progress           int          0–100
├── ResultObjectKey    string       криптослучайный opaque-ключ объекта в MinIO (256-бит); назначается при создании и стабилен на всё время жизни задачи (повтор перезаписывает тот же объект, без «осиротевших» файлов); не раскрывается клиенту; скачивается только при Status == Done
├── Error              string?      стандартизированный текст ошибки при Failed (без деталей инфраструктуры; полное исключение пишется в лог)
├── Attempts           int          число сделанных попыток; падение переводит в Failed только после исчерпания MaxAttempts (= 3)
├── CreatedAt          DateTimeOffset
...
Упавшая попытка не финальна: задача остаётся `Running` и перепланируется (back-off, как у `ReceiptFetchJob`) до `MaxAttempts`, после чего становится терминально `Failed` — UI всё это время показывает «в процессе», а по достижении терминального статуса получает корректный `Done`/`Failed`.
...
**Story 6.1: Импорт из ФНС Excel (асинхронно)**
- [x] T6.1.1 Парсер Excel-формата (.xlsx) выгрузки чеков приложения «Налоги ФЛ» — ФНС теперь отдаёт `.xlsx`, не JSON (`ClosedXmlFnsReceiptParser`: столбцы по заголовкам, плоский лист «строка = позиция», группировка по «Номер чека», дедуп по `ExternalNumber + ИНН + дата`; каждый чек → Expense-начисление + связанный `Receipt` с `ReceiptItem`)
- [x] T6.1.2 POST /accruals/import → загрузка файла в MinIO + фоновая задача (Hangfire) → возвращает `jobId`
- [x] T6.1.3 Сущность `BackgroundTask` (статус/прогресс) + GET /jobs/{id} — реализовано вместе со Story 6.2 (общая инфраструктура задач); результат импорта — JSON-сводка `ImportSummary` через GET /jobs/{id}/result. Осталась только UI (6.1.4)
- [x] T6.1.4 UI: загрузка файла + опрос статуса задачи + отображение результата — страница `/import` (`pages/import`, фича `accrual-import`): выбор `.xlsx`, запуск фоновой задачи, живой статус и сводка (`ImportSummary` с предупреждениями). Завершение обрабатывается глобально (Story 6.3)

> **Ограничения импорта (осознанные):**
> - **Без провайдера.** Импорт не вызывает ProverkaCheka — работает и без токена; `capabilities.fnsImport` всегда `true`.
> - **Только рубли.** Импортированные начисления создаются как `RUB` без курса. Для пользователя с базовой валютой ≠ RUB сумма пока **не конвертируется** (считается 1:1 в базовой валюте). Пересмотреть, когда импорт понадобится не-RUB пользователям.
> - **Время без зоны.** В выгрузке нет таймзоны (локальное МСК); дата сохраняется как UTC «как есть», поэтому момент импортированного чека может отличаться на смещение МСК от отсканированного по QR.
> - **Ошибки по строкам видны пользователю.** Структурно нечитаемый файл → терминальный фейл; отдельная битая строка пропускается и попадает в `ImportSummary.Warnings`/`RowsFailed`.
> - **Атомарность.** Парсинг + вставка начислений/чеков + завершение задачи коммитятся одним `SaveChangesAsync` (сводка грузится первой) — транзиентный сбой не оставляет полу-импортированных строк, повтор пересчитывает корректно. Гонку параллельных задач страхует partial-unique индекс `receipts (UserId, ExternalNumber, INN, Date) WHERE ExternalNumber IS NOT NULL`.

**Story 6.2: Экспорт в CSV (асинхронно)**
- [x] T6.2.1 POST /accruals/export (применяет фильтры) → фоновая задача → CSV в приватный бакет MinIO (object key — криптослучайный токен)
- [x] T6.2.2 GET /jobs/{id}/result — стрим файла через сервис: проверка владельца (`BackgroundTask.UserId`) + rate-limit; presigned URL не используем
- [x] T6.2.3 Кнопка «Экспорт» в списке: запуск задачи + уведомление о готовности

**Story 6.3: Глобальное отслеживание фоновых задач на фронте**
- [x] T6.3.1 Глобальный стейт активных задач экспорта/импорта (`BackgroundTask`), не привязанный к странице. Реализовано: персистентный Zustand-стор `useBackgroundTasks` (`entities/background-task`, `localStorage`), app-level драйвер `BackgroundTaskTracker` в `AppProviders` (по `JobWatcher` на задачу — опрос идёт на **любой** странице), глобальные тосты `Done`/`Failed`, авто-скачивание готового CSV и сводка импорта, индикатор активных задач в шапке (`BackgroundTasksIndicator`). Переживает перезагрузку вкладки (восстановление по сохранённым `jobId`; флаг `handled` не даёт повторно скачать уже выданный файл, но добивает задачу, завершившуюся при закрытой вкладке) и очищается при смене пользователя. Экспорт переведён со страничного стейта на общий стор. Охватывает и экспорт (6.2), и импорт (6.1.4).
...
**Story 7.2: Переработка UI Журнала _(на потом)_**
- [ ] T7.2.1 Переработать UI страницы журнала (`frontend/src/pages/journal/ui/JournalPage.tsx`) — улучшить читаемость/навигацию
...
- [ ] T8.1.5 (на потом) Автоподстановка курса из ЦБ РФ (cbr.ru) — заполнять `ExchangeRate` в форме автоматически по дате операции, с кэшем (FusionCache) и фолбэком на ручной ввод. Сейчас курс вводится только вручную
...
### Epic 11: Администрирование (multi-user) _(поздний этап)_

> Только для роли admin (`ICurrentUserService.IsAdmin`). Админ обходит EF global query filter, поэтому видит данные всех юзеров. Обычный пользователь **никогда** не должен видеть чужие данные — колонки/фильтры по юзеру гейтить по роли. `DisplayName` берётся из `public.user_profiles`, id — `auth.users.id`. Для пикера юзера переиспользовать общий themed `Select`.

**Story 11.1: Отображение владельца начисления**
- [ ] T11.1.1 Показывать `DisplayName` юзера в списке начислений и на главной (только для админа)
- [ ] T11.1.2 Показывать `id` юзера приглушённо внутри начисления (строка списка/детальная)

**Story 11.2: Фильтрация по юзеру**
- [ ] T11.2.1 Админ может фильтровать начисления по юзеру (фильтр + бэкенд-поддержка)
- [ ] T11.2.2 Аналитика/дашборд для админа фильтруется по юзеру
...
| Feature-gate загрузки чеков по токену провайдера | Нет токена ProverkaCheka → сканирование отключено целиком: `scan-qr` отдаёт `503` с кодом `receipt_scanning_disabled`, фоновые чеки в очереди ставятся на паузу (не жгут retry-бюджет), а `GET /api/v1/capabilities` (`{ receiptScanning, fnsImport }`) сообщает фронту, что выключить. **Флаг проверяется в рантайме**: токен можно добавить на бэке независимо от деплоя фронта, поэтому SPA не хардкодит доступность, а опрашивает `capabilities`. На токен завязан **только** `receiptScanning`. Импорт из ФНС провайдер не вызывает (парсит загруженный файл), поэтому `fnsImport` от токена **не зависит** и всегда `true` — импорт работает и без токена ProverkaCheka. |
...
- **Стабильный object key (повторы без «сирот»):** `ResultObjectKey` генерируется один раз при создании задачи и неизменен — повторный/возобновлённый прогон джобы заливает в тот же ключ (перезапись), а не плодит новый объект. Ключ остаётся **криптослучайным** (не выводится из перебираемого `id`), поэтому слой неугадываемости файла сохраняется.
- **Ограниченные повторы + стандартизированная ошибка:** транзиентный сбой экспорта считается попыткой (`Attempts`), задача перепланируется (back-off) до `MaxAttempts`, затем — терминальный `Failed`. Сырой текст исключения **никогда** не отдаётся клиенту: он логируется, а в `Error` кладётся обобщённое сообщение.
- **TODO (масштаб):** сейчас CSV формируется целиком в памяти (`byte[]`) и затем заливается в MinIO. Для больших выгрузок в будущем — стримить генерацию по частям (пишем в MinIO потоково / multipart-upload, не материализуя весь файл). Скачивание уже потоковое.
