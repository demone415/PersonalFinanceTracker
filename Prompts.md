...
Делаем `Story 6.1: Импорт из ФНС JSON (асинхронно)`. ФНС поменяла свой импорт: теперь она отдает Excel файл. Я приложил его для примера в корень - `Receipts (3).xlsx`.  Прочти его структуру. Столбец "Номер чека" = "ExternalNumber" в "ReceiptItems" в приложении. По нему можно проверять есть ли такая запись уже в бд, чтобы не плодить дубли.

---

> Каждая группа строк (один чек) из Excel — во что превращается при импорте?

Начисление + чек с позициями

---

> Ключ дедупликации. «Номер чека» (ExternalNumber) не уникален между магазинами (в файле есть «39», «49», «1»). Как сравнивать с существующими?

ExternalNumber + ИНН + дата
...
Проверь сделанный тобой импорт на соответствие документации по `Story 6.1: Импорт из ФНС JSON (асинхронно)`. Исправь там только первую таску, так как мы не json импортировали, а excel. Отметь что сделано
...
/ecc:review-pr #20. Кроме всего прочего проверь его на соответствие требованиям из ARCHITECTURE.md

---

Пункт 1 - Импорт не использует провайдер чеков, так как на вход подается файл. Так что функция должна работать, даже когда токена провайдера нет. Нужно отметить это в документации
2 - Начисления создаваемые из импорта пока что всегда будут в рублях. Пока не знаю, что делать с юзерами, у которых дефлотная валюта не рубли. Оставим как есть. Отметь это в документации.
3 - Нужно, чтобы ошибки парсинга показывались юзеру. Пусть в результат джобы записываются ошибки
4 - Отметь это в документации
5 - Это надо поправить
6 - Сделай индекс как предложил

---

/goal Теперь делаем фронтенд часть для импорта из ФНС, учитывая глобальное отслеживание фоновых задач
Выполни ` T6.1.4 UI: загрузка файла + опрос статуса задачи + отображение результата` вместе с `Story 6.3: Глобальное отслеживание фоновых задач на фронте (на потом)`

---

Закомить это в отдельную ветку и создай ПР

---

/ecc:review-pr #21. Кроме всего прочего проверь его на соответствие требованиям и архитектурным решениям из ARCHITECTURE.md

---

По пунктам:
1 - исправь как предложил
2 - исправь как предложил
3 - Таска должна показываться только юзеру, который ее запустил. Исправь

---

Комить и мержи в main

---

Создай readme.md c описанием проекта, зависимостей и инструкцией к запуску

---

Надо открыть регистрацию в сервисе. Сделай UI форму регистрации через GoTrue, если ее еще нет. Добавь возможность юзеру зарегистрироваться на форме входа

---

Мержи в мейн и обнови claude.md

---

Надо расширить seed данные.  Должно быть транзакций за 6 месяцев, 12+ категорий, 3+ бюджетов

---

Давай увеличим доходы, чтобы финальный баланс каждого месяца был в плюсе
...
Комить, пуш и мержи в main

---

После успешного импорта на фронте показывается пустая страница.
backend отдает

```
{
    "id": "019ee961-df04-7e03-8160-14b156204095",
    "type": "ImportFns",
    "status": "Done",
    "progress": 100,
    "error": null,
    "createdAt": "2026-06-21T08:52:45.957163+00:00",
    "completedAt": "2026-06-21T08:52:46.706803+00:00",
    "hasResult": true
}
```

Но на фронте страница очищается и становится пустой

---

Нашел в console ошибку `Minified React error #185`
