...
Импорт начислений не отображается в Журнале

---

/ui-ux-pro-max 
В журнале поля сущностей написаны на английском. Юзеру это не будет понятно. Спроектируй этот UI получше.

* Должна быть возможность перейти в начисление по клику
* Должно быть понятно, что поменялось.
* Должны быть видны изменения позиций (хотя бы добавление и удаление их из списка)
* Поля написаны по-русски

---

На каждое начисление и бюджет в seed данных должна быть запись в журнале

---

Проверь приложение на соотвествие требованиям:
...
Создай github ci pipeline, где будут lint + тесты + сборка
