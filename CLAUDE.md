...
- **Optimistic locking**: mutable user entities (`Accrual`, `MonthlyBudget`) use the PostgreSQL `xmin` system column as concurrency token — `builder.Property<uint>("xmin").HasColumnType("xid").HasColumnName("xmin").ValueGeneratedOnAddOrUpdate().IsConcurrencyToken()`. No migration column is added (xmin is a system column). Mirror this when adding a new editable entity.
- **Error mapping is centralized**: services throw domain exceptions; `GlobalExceptionHandler` maps them — `NotFoundException`→404, `ForbiddenAccessException`→403, `ConflictException`→409, `DbUpdateConcurrencyException`→409, `FeatureDisabledException`→503. Don't emit status codes ad-hoc from controllers.
- **Currency aggregation contract**: until multi-currency (Epic 8) lands, every aggregate (dashboard, budget progress) sums `Amount` **as stored** — treated as the user's base currency; `Currency`/`ExchangeRate` are reference-only and excluded from sums. When conversion arrives, apply it in **all** aggregation sites at once (`DashboardService`, `BudgetService.GetProgressAsync`) so figures stay consistent.
