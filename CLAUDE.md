...
- **Scope uniqueness and "my-data" reads by `UserId` explicitly** — the global filter is *bypassed for admins*, so a uniqueness/duplicate check (e.g. one budget per category/month) or a per-caller endpoint (e.g. `GetProgressAsync`) that relies on the filter alone will leak cross-user rows or raise false conflicts when the caller is an admin. Add `b.UserId == currentUser.UserId` to those queries.
...
- **Public signup is open**: anyone can self-register from the SPA via the GoTrue signup form (`features/auth` `RegisterForm` → `supabase.auth.signUp` → `POST /auth/v1/signup`; `/register` route, cross-linked from `/login`). Enabled by `GOTRUE_DISABLE_SIGNUP=false` in `docker-compose.yml`. `GOTRUE_MAILER_AUTOCONFIRM=true` → no email confirmation, no captcha, so a successful signup returns a live session and the user is signed in immediately. Self-registered users get **no `app_metadata.role`**, so both the SPA and `ICurrentUserService` treat them as regular `user`s; **admins are still provisioned only via the GoTrue Admin API** (role set in `app_metadata.role`). Rate-limit both the login and the signup endpoints.
...
- **Async import/export**: import/export are **always async** — `POST /accruals/import` and `POST /accruals/export` enqueue a Hangfire job, return `{ jobId }` (202). Status via `GET /jobs/{id}`. The result is **streamed through the service** at `GET /jobs/{id}/result` — **no presigned URLs**: the backend streams the object from a private MinIO bucket, enforces ownership via `BackgroundTask.UserId`, and the endpoint is rate-limited. State lives in `BackgroundTask`. Never do import/export inline in the request. Jobs run on a shared Hangfire server bound to the `exports` + `imports` queues (kept off the single-worker `receipts` queue); the off-request body lives in a transport-agnostic, idempotent `*Processor` (a Done/Failed task is a no-op) with a bounded retry budget (`BackgroundTask.MaxAttempts`).
- **FNS import is Excel, not JSON (Story 6.1)**: the FNS «Налоги ФЛ» export is now an **`.xlsx`** workbook, uploaded as `multipart/form-data` (`IFormFile file`) to `POST /accruals/import`. The producer (`AccrualImportService`) stores the source workbook under a private `imports/src/...` key and enqueues the job; the off-request `AccrualImportProcessor` parses it via `IFnsReceiptParser` (ClosedXML — `ClosedXmlFnsReceiptParser`, columns located by **header name** so column order is tolerant). The sheet is **flat — one row per line item** — with receipt-level columns (`Дата и время чека`, `Итого по чеку`, `ИНН продавца` [org name + INN bundled], `Место расчета`, `Номер чека`) repeated across a receipt's rows; rows are grouped by `Номер чека`. Each group → an **Expense `Accrual` + a linked `Receipt` (FetchStatus=Fetched) with `ReceiptItem`s**, accrual `Amount` = «Итого по чеку». **Dedup is per-user on `(ExternalNumber, INN, Date)`** — `ExternalNumber` (=`Номер чека`) alone is not unique across merchants; enforced both in the processor and by a **partial unique index** on `receipts (UserId, ExternalNumber, INN, Date) WHERE ExternalNumber IS NOT NULL` (so concurrent import jobs can't double-insert; QR receipts with a null `ExternalNumber` are exempt). The job result is a **JSON `ImportSummary`** (imported / skipped-duplicate / total / **`RowsFailed` + `Warnings`** — per-row parse problems are surfaced here, not swallowed) at `GET /jobs/{id}/result`. A structurally unreadable file fails **terminally** (`FnsImportFormatException` → no retry); an individual malformed row is skipped with a warning. The parse + accrual/receipt insert + task completion commit in **one** `SaveChangesAsync` (summary uploaded first) so a transient failure never leaves half-imported rows behind an unfinished task and a retry re-runs with accurate counts; transient failures use the retry budget. `BackgroundTaskType.ImportFns` (was `ImportJson`). **Not gated by the provider token**: import parses a user file and never calls ProverkaCheka, so `GET /capabilities` reports `fnsImport: true` always (only `receiptScanning` depends on the token). **Imports are RU-only for now**: imported accruals are created as `RUB` with no exchange rate, so a user whose base currency ≠ RUB will have the amount counted 1:1 in their base currency (not converted) — acceptable for now, revisit if non-RUB bases must import. **Dates carry no time-zone** in the export (local MSK wall-clock); they are stored labeled UTC verbatim, so an imported receipt's instant can differ by the MSK offset from a QR-scanned one.
